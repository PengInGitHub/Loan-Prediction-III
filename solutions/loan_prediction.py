...
solution_path = "/Users/pengchengliu/Documents/GitHub/Loan_Prediction/solution/"
...
#custom method listdir_nohidden instead of os.listdir, because os.listdir may run into hidden files problem        
...
files = listdir_nohidden('featurescore')
#save into a dict
fs = {}
for f in files:
...
train = original
train_x = pd.read_csv(data_path+"train_x_discretization.csv")
train_y = train[['Loan_ID']+['Loan_Status']]
train_y['Loan_Status'] = train_y['Loan_Status'].map({'Y': 0, 'N': 1})

train_xy = pd.merge(train_x,train_y,on='Loan_ID')
train_xy, test_xy = train_test_split(train_xy, test_size = 0.2)
train_y = train_xy[['Loan_ID']+['Loan_Status']]
test_y = test_xy[['Loan_ID']+['Loan_Status']]
#flabel of xgb
